...
async def log_current_balance(api, label="balance_check"):
    try:
        balance_response = await api.balance()
        balance_data = balance_response.get("balance", {})
        print(f"Current Balance: {balance_data.get('balance')}")
        log_event(label, {
    ...
    try:
        ...
        while True:
            ...
            try:
                ...
                exception_count += 1
                log_event("trade_error", {"error": str(e), "exception_count": exception_count})
                print(f"[trade] Error #{exception_count}: {e}")
                if exception_count >= MAX_EXCEPTIONS:
                    await reset_bot()
                # Optionally re‐raise or return so the loop in main can continue cleanly
                return
            else:
                # if trade succeeds, reset the counter
                exception_count = 0
        ...
        while True:
            ...
            if poc_data.get("is_sold"):
                ...
                await log_current_balance(api, label="post_trade_balance")
                break
...
async def fetch_historical_data(symbol, count, retries=3, delay=2):
    """
    Fetch historical tick data with retries in case of transient errors like 'WrongResponse'.
    """
    for attempt in range(retries):
        try:
            async with websockets.connect(f'wss://ws.binaryws.com/websockets/v3?app_id={app_id}') as websocket:
                ...
                log_event("historical_data_fetched", {
                    "symbol": symbol,
                    "count": count,
                    "data_summary": {
            ...
            print(f"[Attempt {attempt+1}] Failed to fetch historical data: {e}")
            log_event("historical_data_retry_error", {
                "attempt": attempt + 1,
                "error": str(e)
            })
            if attempt < retries - 1:
                ...
                raise Exception(f"Error fetching historical data after {retries} attempts: {e}")
...
async def reset_bot():
    ...
    script_path = os.path.abspath(__file__)  # gets full path to current script
...
async def main():
    ...
    try:
        ...
        await log_current_balance(api, label="initial_balance")
        while True:
