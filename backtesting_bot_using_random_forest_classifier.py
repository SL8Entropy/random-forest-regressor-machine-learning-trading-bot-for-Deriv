...
# --- Model Loading/Training Block (Revised) ---
feature_columns = ['close', 'RSI_7', 'RSI_14', 'RSI_21', 'Stochastic %K', 'Stochastic %D']
barrier = 0.1 # Make sure barrier is defined before use

# Initialize X with the feature columns so it's always available.
# This creates an empty DataFrame with the correct column names.
...
if os.path.exists(model_file_path):
    ...
    try:
        ...
        # Create the target variable 'Y' for classification
        data_with_indicators_list['next_close'] = data_with_indicators_list['close'].shift(-1)

        def classify_price_movement(row):
            if pd.isna(row['next_close']):
                return 'Neutral'
            if row['next_close'] > row['close'] + barrier:
                return 'Up'
            elif row['next_close'] < row['close'] - barrier:
                return 'Down'
            else:
                return 'Neutral'

        data_with_indicators_list['Y'] = data_with_indicators_list.apply(classify_price_movement, axis=1)

        # Drop the last row as it will have NaN for 'next_close' and 'Y' if not handled
        data_with_indicators_list.dropna(subset=['next_close', 'Y'], inplace=True)

        # Ensure X is an explicit copy here to prevent SettingWithCopyWarning
        X = data_with_indicators_list[feature_columns].copy() 
        Y = data_with_indicators_list['Y']

        X.fillna(0, inplace=True)
        if len(X) > len(Y): # This condition might be less relevant with classification Y creation
            X = X.iloc[:len(Y)]
...
# The rest of your script (trading parameters, strategy function etc.) remains the same.
...
def enhanced_triple_rebound_strategy(rsi_values, stoch_k, stoch_d, data, log_time, barrier=0.01):
    ...
    # Ensure the DataFrame columns match the model's expected feature columns (X.columns)
    latest_data = pd.DataFrame([latest_data_list], columns=X.columns)
    
    # Predict the class ('Up', 'Down', 'Neutral') using the classifier
    prediction_class = model.predict(latest_data)[0]
    
    # Get prediction probabilities for more nuanced decision-making (optional but good practice)
    # prediction_probabilities = model.predict_proba(latest_data)
    # class_labels = model.classes_
    # print(f"Prediction Probabilities: {dict(zip(class_labels, prediction_probabilities[0]))}")

    print(f"RSI (7,14,21): {rsi_7:.2f}, {rsi_14:.2f}, {rsi_21:.2f}")
    print(f"Stochastic (K,D): {stoch_k:.2f}, {stoch_d:.2f}")
    print(f"Predicted movement after 1 minute = {prediction_class}")
    ...
    log_event("prediction", {
        ...
        "stoch_k": stoch_k, 
        "stoch_d": stoch_d, 
        "predicted_movement": prediction_class
    }, log_time)

    decision = None

    # Adjust the conditions based on the predicted class and your strategy
    if prediction_class == 'Up' and \
       rsi_7 < Lowamount and rsi_14 < Lowamount and rsi_21 < Lowamount + 5 and \
       stoch_k < Lowamount and stoch_d < Lowamount:
        print("Decision: Betting UP (CALL)")
        decision = "CALL"
    elif prediction_class == 'Down' and \
         rsi_7 > Highamount and rsi_14 > Highamount and rsi_21 > Highamount - 5 and \
         stoch_k > Highamount and stoch_d > Highamount:
